...
const Dialog = () => {
  ...
  const handleDownloadPdf = () => {
    if (title) {
      const chatbotId = title.replace(/\s/g, "_").toLowerCase();
      PdfService.downloadHistory(chatbotId);
    } else {
      alert("Guarde la sala primero antes de descargar el historial.");
    }
  };
      ...
      <div className="ds-dl">
        ...
        <form
          ...
          <footer className="ds-dl-footer">
            <button 
              className="ds-dl-f-button"
              type="button" 
              onClick={handleDownloadPdf}
              disabled={!id}
              title="Descargar Historial de Chat"
              style={{ background: 'var(--silver-foil)', color: 'var(--slate-900)' }}
            >
              <span className="ds-dl-f-b-icon icon-download"></span>
              Descargar PDF
            </button>
